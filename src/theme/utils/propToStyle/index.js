...
(que são as midiaQueries). Os breakpoints são objetos (que contem o sm, md, lg e etc).
Mas a leitura de CSS não lê objetos - exemplo: textAling: right;. ou seja, o css só lê strings,
então temos que transformar o objeto em string para que o css possa ler.E, no final, se retornar
uma string então é só ler [propname]: props[propname], que seria, por exemplo, textAlign: right */

// JAVASCRIPT => Estamos utilizando o conceito de CLOSURE.
// A função propToStyle recebe um parametro que definimos como propName.
// Vamos (exemplo) chamar essa função com propToStyle('margin'). Ela retorna outra função que
// utiliza de seu escopo e só vale dentro dela (isso é o conceito de Closure).
